...
export class CreateOrEditEmployeeComponent implements OnInit {
  ...
  @Output('modalClose') modalClose = new EventEmitter();
  ...
  save() {
    ...
    this.employee.ImageString = this.url.substring(23, this.url.length);
    if(this.action ==2){
      this._employeeService.registerEmployee(this.employee).subscribe(res => {
  ...
  checkValidate() {
    if (!this.employee?.EmpCode) {
      alertify.error('Mã nhân viên không được trống!');
      return false;
    };
    ...
    if (this.employee?.IsDeleted == undefined) {
      alertify.error('Trạng thái không được trống!');
      return false;
    };
    
    if (this.employee?.Email && !this.emailValidate(this.employee?.Email)) {
      alertify.error('Email không hợp lệ');
      return false;
    }
    if (!this.employee?.Tel) {
      alertify.error('SĐT không được trống!');
      return false;
    };

    if(!this.phoneNumberValidate(this.employee?.Tel)) {
      alertify.error('Số điện thoại không hợp lệ');
      return false;
    };

    if (!this.employee?.Cmnd) {
      alertify.error('Chứng minh thư không được trống!');
      return false;
    };
    if (!this.employee?.Username) {
      alertify.error('Tài khoản không được trống!');
      return false;
    };
    ...
    if (this.employeeList.some(e => e.Username.toLowerCase() == this.employee.Username.toLowerCase() ) && this.action ==2 ||
    this.employeeList.some(e => e.Username.toLowerCase() == this.employee.Username.toLowerCase() && e.Id != this.employee.Id ) && this.action == 3 ) {
      alertify.error('Tài khoản đã tồn tại!');
      return false;
    }
    if (this.employeeList.some(e => e.EmpCode.toLowerCase() == this.employee.EmpCode.toLowerCase() ) && this.action ==2 ||
    this.employeeList.some(e => e.EmpCode.toLowerCase() == this.employee.EmpCode.toLowerCase() && e.Id != this.employee.Id ) && this.action == 3 ) {
      alertify.error('Mã nhân viên đã tồn tại!');
      return false;
    }
  ...
  selectFile(event: { target: { files: Blob[]; }; }) {
    ...
    reader.onload = (e) => {
      ...
      this.employee.ImageString = this.url;
    }
  }
