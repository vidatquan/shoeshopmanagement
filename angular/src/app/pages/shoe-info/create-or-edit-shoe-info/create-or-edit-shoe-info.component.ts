import { Component, EventEmitter, Input, OnInit, Output, ViewChild } from '@angular/core';
import { SafeResourceUrl } from '@angular/platform-browser';
import * as moment from 'moment';
import { ModalDirective } from 'ngx-bootstrap/modal';
import { finalize } from 'rxjs/operators';
import { GetShoeInfoInput } from 'src/app/_models/shoe-info/GetShoeInfoInput';
import { HistoryShoePrice } from 'src/app/_models/shoe-info/HistoryShoePrice';
import { Shoes } from 'src/app/_models/shoe-info/shoes';
import { ShoesService } from 'src/app/_services/shoes.service';
declare let alertify: any;
@Component({
  selector: 'app-create-or-edit-shoe-info',
  templateUrl: './create-or-edit-shoe-info.component.html',
  styleUrls: ['./create-or-edit-shoe-info.component.scss']
})
export class CreateOrEditShoeInfoComponent implements OnInit {

  @ViewChild('modal') public modal: ModalDirective;
  // @ViewChild('imgContainer') imgContainer: { nativeElement: { getBoundingClientRect: () => { (): any; new(): any; height: number; width: number; }; }; } | undefined;
  @Output() modalSave: EventEmitter<any> = new EventEmitter<any>();
  url: SafeResourceUrl | any | undefined;
  colorList: any[]= [
    {label:"Đen",value:"Đen"},
    {label:"Đỏ",value:"Đỏ"},
    {label:"Xám",value:"Xám"},
    {label:"Hồng",value:"Hồng"},
    {label:"Trắng",value:"Trắng"},
    {label:"Xanh da trời",value:"Xanh da trời"},
    {label:"Xanh rêu",value:"Xanh rêu"},
    {label:"Phối màu",value:"Phối màu"},
  ];
  statusList: any[]= [
    {label:"Hoạt động",value:0},
    {label:"Không hoạt động",value:1},
  ];
  genderList: any[]= [
    {label:"Nam",value:0},
    {label:"Nữ",value:1},
    {label:"Unisex",value:2},
    {label:"Trẻ em",value:3},
  ];
  typeList: any[]= [
    {label:"UltraBoost",value:0},
    {label:"Yeezy",value:1},
    {label:"StanSmith",value:2},
    {label:"NMD",value:3},
    {label:"Alphabounce",value:4},
    {label:"EQT",value:5},
    {label:"PulseBoost",value:6},
    {label:"SuperStar",value:7}
  ];
  sizeList: any[]= [
    {label:"3",value:3},
    {label:"4",value:4},
    {label:"5",value:5},
    {label:"6",value:6},
    {label:"7",value:7},
    {label:"8",value:8},
    {label:"9",value:9},
    {label:"10",value:10},
    {label:"11",value:11},
    {label:"24",value:24},
    {label:"28",value:28},
    {label:"30",value:30},
    {label:"36",value:36},
    {label:"36 1/2",value:3612},
    {label:"37",value:37},
    {label:"37 1/2",value:3712},
    {label:"38",value:38},
    {label:"38 1/2",value:3812},
    {label:"39",value:39},
    {label:"39 1/2",value:3912},
    {label:"40",value:40},
    {label:"40 1/2",value:4012},
    {label:"41",value:41},
    {label:"41 1/2",value:4112},
    {label:"42",value:41},
    {label:"42 1/2",value:4212},
    {label:"43",value:41},
    {label:"43 1/2",value:4312},
  ];
  shoe: Shoes = new Shoes();
  fullName: string;
  code: string;
  type: any;
  gender: any;
  size: any;
  color: any;
  action: number; // 1 xem chi tiết, 2 thêm mới, 3 sửa
  shoeList = [];
  
  constructor(private _shoesService: ShoesService) { }

  ngOnInit() {
  }

  close() {
    localStorage.removeItem('oldRealPrice');
    localStorage.removeItem('oldSellPrice');
    this.modal.hide();
  }

  show(action: number, event?) {
    if (action != 1) this.getAllShoesList();
    this.action = action;
    this.url = "assets/img/userIcon.jpg";
    if (action == 1 || action == 3) {
      this.shoe = event;
      this.url = 'data:image/jpeg;base64,' + event.ImageString;
      localStorage.setItem('oldRealPrice', JSON.stringify(this.shoe.RealPrice));
      localStorage.setItem('oldSellPrice', JSON.stringify(this.shoe.SellPrice));
    }
    else if (action == 2) {
      this.shoe = new Shoes();
    }
    this.modal.show();
  }
  getAllShoesList() {
    this.shoeList = [];
    var shoes = new GetShoeInfoInput();
    shoes.ShoeName = '';
    shoes.ShoeCode = '';
    shoes.ShoeSize = -1;
    shoes.Gender = -1;
    shoes.Color = 'Tất cả';
    shoes.ShoeType = -1;
    shoes.IsDeleted = -1;
    this._shoesService.getShoesInfo(shoes).subscribe((res) => {
      this.shoeList = res;
    });
  }

  save() {
    if (!this.checkValidate()) return;
    this.shoe.ImageString = this.url.substring(23, this.url.length);
    if(this.action ==2){
      this.shoe.ModifyPriceTime = moment();
      this._shoesService.createShoesInfo(this.shoe).subscribe(res => {
        alertify.success('Thêm mới thành công');
        this.modalSave.emit(null);
        this.close();
      });
    }
    else if (this.action ==3){
      var body = new HistoryShoePrice();
      if(this.shoe.RealPrice != JSON.parse(localStorage.getItem('oldRealPrice')) || 
      this.shoe.SellPrice != JSON.parse(localStorage.getItem('oldSellPrice'))) 
        {
          body.Id = 0;
          body.ShoeId = this.shoe.Id;
          body.RealPrice = JSON.parse(localStorage.getItem('oldRealPrice'));
          body.SellPrice = JSON.parse(localStorage.getItem('oldSellPrice'));
          body.ApplyDate = this.shoe.ModifyPriceTime;
          this.shoe.ModifyPriceTime = moment();
          this.shoe.ModifyPriceTime.add(7, 'hours');
          body.ExpiryDate = this.shoe.ModifyPriceTime;

          this._shoesService.createHistoryShoePrice(body).pipe(finalize(() => {
            this._shoesService.updateShoesInfo(this.shoe).subscribe(res => {
              alertify.success('Cập nhật thành công');
              this.modalSave.emit(null);
              this.close();
            });
          })).subscribe(() => {
            alertify.success('Cập nhật lịch sử giá thành công');
          });

        }
      else{
        this._shoesService.updateShoesInfo(this.shoe).subscribe(res => {
          alertify.success('Cập nhật thành công');
          this.modalSave.emit(null);
          this.close();
        });

      }
    }
    
  }

  checkValidate() {
    if (!this.shoe?.ShoeCode) {
      alertify.error('Mã giày không được trống!');
      return false;
    };
    if (this.shoe?.IsDeleted == undefined ) {
      alertify.error('Trạng thái không được trống!');
      return false;
    };
    if (this.shoe?.ShoeQty == undefined) {
      alertify.error('Số lượng không được trống!');
      return false;
    };
    if (isNaN(this.shoe?.ShoeQty) || this.shoe?.ShoeQty < 0) {
      alertify.error('Số lượng không hợp lệ!');
      return false;
    };
    if (!this.shoe?.RealPrice) {
      alertify.error('Giá thực không được trống!');
      return false;
    };
    if (isNaN(this.shoe?.RealPrice) || Number(this.shoe?.RealPrice) < 0) {
      alertify.error('Giá thực không hợp lệ!');
      return false;
    };
    if (!this.shoe?.SellPrice) {
      alertify.error('Giá bán không được trống!');
      return false;
    };
    if (isNaN(this.shoe?.SellPrice) || Number(this.shoe?.SellPrice) < 0) {
      alertify.error('Giá bán không hợp lệ!');
      return false;
    };
    if (this.shoe?.SellPrice && this.shoe?.RealPrice && Number(this.shoe?.RealPrice) > Number(this.shoe?.SellPrice)) {
      alertify.error('Giá thực không được lớn hơn giá bán!');
      return false;
    };

    if (this.shoeList.some(e => e.ShoeCode.toLowerCase() == this.shoe.ShoeCode.toLowerCase()) && this.action == 2 ||
    this.shoeList.some(e => e.ShoeCode.toLowerCase() == this.shoe.ShoeCode.toLowerCase() &&  e.Id != this.shoe.Id) && this.action == 3) {
      alertify.error('Mã giày đã tồn tại!');
      return false;
    }

    return true;
  }

  selectFile(event: { target: { files: Blob[]; }; }) {
    var mimeType = event.target.files[0].type;
    if (mimeType.match(/image\/*/) == null) {
      return;
    }

    var reader = new FileReader();
    reader.readAsDataURL(event.target.files[0]);

    reader.onload = (e) => {
      const files = event.target.files;
      if (files) {
        for (const file of files) {
          if (file.size > 1000000) {
            alertify.warn("ảnh của bạn phải nhỏ hơn 1MB");
            return;
          }
        }
      }
      
      this.url = e.target?.result;
      this.shoe.ImageString = this.url;
    }
  }

  defaultPic() {
    return 'data:image/jpeg;base64,' + "";
  }
}
